...
function Get_Score(_judgements, _judgementsWeights, _combosHighest) {
    ...
    let _PScore = ((_HighestP - _HighestPP + Judgements[1])/2)/(_totalNotes*2)
    let _CScore = ((_HighestC - _HighestPP + Judgements[2])/2)/(_totalNotes*3)
    let _HScore = ((_hitNotes - _HighestPP + Judgements[3])/2)/(_totalNotes*4)
